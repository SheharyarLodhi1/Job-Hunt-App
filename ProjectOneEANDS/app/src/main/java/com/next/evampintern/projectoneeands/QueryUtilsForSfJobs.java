...
public class QueryUtilsForSfJobs {

    // tag for the log messages ..
    private static final String QUERY_TAG_SF = QueryUtils.class.getName();

    private QueryUtilsForSfJobs(){}

    /**
     * Query the SearchJobs dataset and return a list of {@link SearchJobs} objects.
     */

    public static List<JobsInSf> fetchSfJobsList(String requestUrl){
        Log.i(QUERY_TAG_SF, "Test: fetchSfJobsList is called");
        ...
        try{
            ...
            Log.i(QUERY_TAG_SF,"Problem Taking the http request", e);
        }
         ...
         *
         */
        List<JobsInSf> list = extractDataFromJson(jsonResponse);
        return list;
    }

    private static List<JobsInSf> extractDataFromJson(String jobsJson) {
        ...
        List<JobsInSf> jobsInSf = new ArrayList<>();
...
//                created_at = formatDate(created_at);
                ...
                jobsInSf.add(new JobsInSf(type, url, created_at, company, company_url, location, title, description));
            }

        }catch (JSONException j){
            Log.i(QUERY_TAG_SF,"There is a problem while parsing the json", j);
        }
    ...
    private static String makeHttpsRequest(URL url) throws IOException {
        ...
        try{
            ...
            if (httpsURLConnection.getResponseCode() == 200){
                ...
                Log.e(QUERY_TAG_SF, "Error response code: " + httpsURLConnection.getResponseCode());
            }
        }catch (IOException io){
            Log.i(QUERY_TAG_SF,"Erro while creating Url", io);{
    ...
    private static URL createUrl(String requestUrl) {
        ...
        try
        {
            ...
            Log.i(QUERY_TAG_SF, "Error in create Url: ", m);
        }
